...
import { normalizeServiceSeoUrl } from "@/data/service-seo";

type ServiceSeoExtrasProps = {
  seo: ServiceSeoEntry;
};

export default function ServiceSeoExtras({ seo }: ServiceSeoExtrasProps) {
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {seo.keywordLinks.map((link) => (
              <Link
                key={link.label}
                href={normalizeServiceSeoUrl(link.url)}
